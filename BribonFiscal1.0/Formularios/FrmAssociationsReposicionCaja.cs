...
{
    public partial class FrmAssociationsReposicionCaja : Form
    {
        // lista de seleccionados
        List<String> uuidLista = new List<String>();

        public FrmAssociationsReposicionCaja()
        {
            InitializeComponent();
        }

        private void FrmAssociationsReposicionCaja_Load(object sender, EventArgs e)
        {
        ...
        {
            ActCFDIAux(this.textBox3.Text.ToString(), this.dateTimePicker3.Value, this.dateTimePicker4.Value);
        }
        private void ActCFDIAux( string rfcreceptor, DateTime fechaini, DateTime fechafin)
        {
            String query = $"call bancos_auxiliarcfdi_pagosvarios('{rfcreceptor}','{fechaini.ToString("yyyy-MM-dd")}','{fechafin.ToString("yyyy-MM-dd")}');";
            DataTable dt = new DataTable();
